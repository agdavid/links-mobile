...
public abstract class QuestionWidget extends LinearLayout {
    ...
    public boolean canGoBack(){
    	Vector<TreeElement> attrs = mPrompt.getBindAttributes();
    	 for (int i = 0; i < attrs.size(); i++) {
    		 if ("canGoBack".equalsIgnoreCase(attrs.get(i).getName())) {
    			 if ("false()".equalsIgnoreCase(attrs.get(i).getAttributeValue()))
                   	 return false;
              	 }
    	 }
    	 return true;
