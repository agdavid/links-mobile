...
import com.nafundi.taskforce.collect.android.listeners.AdvanceToNextListener;
import com.nafundi.taskforce.collect.android.listeners.FormLoaderListener;
...
import com.nafundi.taskforce.collect.android.logic.PropertyManager;
...
import com.nafundi.taskforce.collect.android.tasks.FormLoaderTask;
import com.nafundi.taskforce.collect.android.tasks.SaveToDiskTask;
import com.nafundi.taskforce.collect.android.utilities.FileUtils;
import com.nafundi.taskforce.collect.android.views.ODKView;
import com.nafundi.taskforce.collect.android.widgets.QuestionWidget;
...
public class FormEntryActivity extends Activity implements AnimationListener, FormLoaderListener,
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        ImageButton forwardButton = (ImageButton) findViewById(R.id.form_forward_button);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        if (requestCode == 5555 && resultCode == RESULT_CANCELED) {
            // user hit 'back' from review widget, so step to question before end
        ...
        if (requestCode == 5555 && resultCode == 4321) {
            // user hit 'save and exit'
        
            // the 'end' screen starts the widget, so step to the one before.
            if (mFormController.getEvent() != FormEntryController.EVENT_BEGINNING_OF_FORM) {
                mFormController.stepToPreviousEvent();
            }
            
            // we don't handle the save here because onResume() gets called next.
            // so at the end of onResume() we call finish().
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (mFormController != null) {
            String canJump = mFormController.getModel().getForm().getInstance().getRoot().getAttributeValue(null, "canJump");
            if (canJump == null || !canJump.equalsIgnoreCase("false()")) {
    ...
    private View createView(int event) {
        ...
        switch (event) {
            ...
            case FormEntryController.EVENT_END_OF_FORM:
                String readonly =
                    mFormController.getModel().getForm().getInstance().getRoot()
                            .getAttributeValue(null, "readonly");
                View endView = new View(this);
                if (readonly != null && readonly.equalsIgnoreCase("true()")) {
                    ...
                    // final TextView saveAs = (TextView) endView.findViewById(R.id.save_name);
                    ...
                    final String saveName = mFormController.getFormTitle();
                    ...
                    if (readonly != null && readonly.equalsIgnoreCase("true()")) {
                        ((Button) endView.findViewById(R.id.save_exit_button)).setText("Exit");
                    }
                                ...
                                public void onClick(View v) {
                                    ...
                                    if (saveName.length() < 1) {
                                        Toast.makeText(FormEntryActivity.this,
                                            R.string.save_as_error, Toast.LENGTH_SHORT).show();
                                    } else {
                                        String readonly =
                                            mFormController.getModel().getForm().getInstance()
                                                    .getRoot().getAttributeValue(null, "readonly");
                                        if (readonly != null && readonly.equalsIgnoreCase("true()")) {
                    ...
                    // only show the review widget if form is not readonly                    
                    Intent i = new Intent(this, ReviewWidgetActivity.class);
    ...
    public void showView(View next, AnimationType from) {
        ...
        if (mCurrentView != null) {
            ...
            mQuestionHolder.removeView(mCurrentView);
        }
    ...
    private void createYESNODialog() {
        String[] items = {
            getString(R.string.return_to_survey), getString(R.string.yes_delete)
        };

        mAlertDialog =
            new AlertDialog.Builder(this).setIcon(android.R.drawable.ic_dialog_info)
                    .setTitle(getString(R.string.are_you_sure, mFormController.getFormTitle()))
    ...
    private void discardCurrentInstance() {
        String selection = InstanceColumns.INSTANCE_FILE_PATH + " like '" + mInstancePath + "'";
        Cursor c =
            FormEntryActivity.this.managedQuery(InstanceColumns.CONTENT_URI, null, selection, null,
                null);
        ...
        if (c.getCount() < 1) {
            ...
            if (imageCursor.getCount() > 0) {
                ...
                String id =
                    imageCursor.getString(imageCursor.getColumnIndex(Images.ImageColumns._ID));
            ...
            if (audioCursor.getCount() > 0) {
                ...
                String id =
                    audioCursor.getString(imageCursor.getColumnIndex(Images.ImageColumns._ID));
            ...
            // video
            Cursor videoCursor =
                getContentResolver().query(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, projection,
                    where, null, null);
            if (videoCursor.getCount() > 0) {
                videoCursor.moveToFirst();
                String id =
                    videoCursor.getString(imageCursor.getColumnIndex(Images.ImageColumns._ID));
    ...
    protected void onPause() {
        dismissDialogs();
        // make sure we're not already saving to disk. if we are, currentPrompt is getting
        // constantly updated
    ...
    protected void onResume() {
        ...
        if (returnFromReview) {
            // we put the save here instead of ActivityResult() to avoid a finish()/resume() 
            // race condition
            final String saveName = mFormController.getFormTitle();
            saveDataToDisk(EXIT, true, saveName);
        ...
        switch (saveStatus) {
            ...
            case SaveToDiskTask.SAVE_ERROR:
                Toast.makeText(this, getString(R.string.data_saved_error), Toast.LENGTH_LONG)
                        .show();
                break;
            case SaveToDiskTask.DB_ERROR:
            	Toast.makeText(this, mSaveToDiskTask.getSaveError(), Toast.LENGTH_LONG)
                        .show();
            	break;
            case FormEntryController.ANSWER_CONSTRAINT_VIOLATED:
            case FormEntryController.ANSWER_REQUIRED_BUT_EMPTY:
...
//        }
    ...
    private void sendSavedBroadcast() {
        Intent i = new Intent();
        i.setAction("com.nafundi.taskforce.collect.android.FormSaved");
